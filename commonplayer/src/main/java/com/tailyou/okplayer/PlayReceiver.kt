...
abstract class PlayReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
            prefixAction + Constants.ACTION_S_2_A_PRG -> {
                val duration = intent.extras[Constants.EXTRA_DURATION] as Int
                val progress = intent.extras[Constants.EXTRA_PLAY_PRG] as Int
                onProgressUpdate(duration, progress)
            }
    ...
    abstract fun onProgressUpdate(duration: Int, progress: Int)//同步后台播放进度
